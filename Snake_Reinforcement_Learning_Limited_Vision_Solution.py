...
Vision_Square = 1 #How much the snake should be able to see around its head
NUM_ACTIONS = int(4) #UP, DOWN, LEFT, RIGHT


NUM_BUCKETS = []
for i in range( ((Vision_Square*2)+1) * ((Vision_Square*2)+1)):
    NUM_BUCKETS.append(3)
NUM_BUCKETS.append(2)
NUM_BUCKETS.append(2)
...
def get_state(snake_body, fruit_position):
    head = snake_body[0]
    state_index = []
    #we have to check the positions around the head to see if there is a body part/wall (hazard) or the food
    for y in range(-1, 2):
        for x in range(-1, 2):
            hazard = False
            food = False

            for count in range(1, Vision_Square+1):
                position = [head[0]+(x*10*count), head[1]+(y*10*count)] #get the position we are looking at
                if position[0] == fruit_position[0] and position[1] == fruit_position[1]: #is the block where the food is
                    food = True
                
                if position[0] < 0 or position[0] > window_x-10: #if it's less than our min or greater than our max
                    hazard = True
                if position[1] < 0 or position[1] > window_y-10: #if it's less than our min or greater than our max
                    hazard = True
                for block in snake_body: #check if body is in that position
                    if block[0] == position[0] and block[1] == position[1]:
                        hazard = True
    ...
    if head[0] - snake_body[1][0] > 0: #moving right
        if head[0] > fruit_position[0]:
            state_index.append(1)
        else:
            state_index.append(0)
    elif head[0] - snake_body[1][0] < 0: #moving left
        if head[0] < fruit_position[0]:
            state_index.append(1)
        else:
            state_index.append(0)
    ...
    if head[1] - snake_body[1][1] >= 0: #moving down
        if head[1] < fruit_position[1]:
            state_index.append(1)
        else:
            state_index.append(0)
    elif head[1] - snake_body[1][1] < 0: #moving up
        if head[1] > fruit_position[1]:
            state_index.append(1)
        else:
            state_index.append(0)
...
def train():
    ...
    #initialize our game
        ...
        # initial score
        score = 0        
        state_0 = get_state(snake_body, fruit_position) #get our initial state
        time = 0
        while True:
            time +=1
            reward = 0
            ...
            #get our new state
            state = get_state(snake_body, fruit_position)
...
def test(q_table):
    ...
    #initialize our game
        ...
        while True:
            ...
            if show_vision:
                #draw a transparent gray circle around the snake head
                s = pygame.Surface(( (((Vision_Square*2)+1)*10), (((Vision_Square*2)+1)*10)))  # the size of your rect
                s.set_alpha(128)                # alpha level
                s.fill((255,255,255))           # this fills the entire surface
                game_window.blit(s, (snake_body[0][0]-(Vision_Square*10),snake_body[0][1]-(Vision_Square*10)))    # (0,0) are the top-left coordinates

            state_0 = get_state(snake_body, fruit_position)
